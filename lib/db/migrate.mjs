...
async function tryMigrateWithUrl(connectionString) {
  ...
  try {
    const hasConversations = await tableExists(pool, "conversations");
    const hasProfiles = await tableExists(pool, "profiles");
    const hasProducts = await tableExists(pool, "products");
    const hasAddresses = await tableExists(pool, "user_addresses");

    if (hasConversations && hasProfiles && hasProducts && hasAddresses) {
      return { ok: true };
    }

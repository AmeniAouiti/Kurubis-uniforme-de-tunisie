...
import { Mail, Phone, Package, Send } from "lucide-react";
...
export default function AdminDevisPage() {
  ...
  const [selectedId, setSelectedId] = useState(devisList[0]?.id ?? "");
  ...
  useEffect(() => {
    if (devisList[0] && !selectedId) setSelectedId(devisList[0].id);
  }, [devisList, selectedId]);
  ...
  return (
    <AdminPage title="Demandes de devis" subtitle={`${pendingQuotes} en attente`}>
            ...
            {devisList.map((conv) => (
              <li key={conv.id}>
                ...
                >
                  ...
                  <p className="text-xs text-muted">{conv.quoteItems.length} article(s)</p>
                </button>
              </li>
            ))}
          </ul>
            ...
            <>
              ...
              <div className="p-4 border-t space-y-4">
                ...
                <form onSubmit={handleReply} className="flex gap-2">
                  <Textarea value={reply} onChange={(e) => setReply(e.target.value)} rows={2} className="flex-1" placeholder="Répondre au client" />

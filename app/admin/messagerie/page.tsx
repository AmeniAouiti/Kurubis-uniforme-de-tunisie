...
export default function AdminMessageriePage() {
  ...
  useEffect(() => {
    if (sorted[0] && !selectedId) setSelectedId(sorted[0].id);
  }, [sorted, selectedId]);
  ...
  return (
    <AdminPage
      title="Messagerie"
      subtitle={`${unreadByAdmin} non lue(s) · Temps réel via Socket.io`}
    >
          ...
          {!selected ? (
            ...
            <>
              ...
              <form onSubmit={handleReply} className="p-4 border-t border-border">
                <Textarea value={reply} onChange={(e) => setReply(e.target.value)} rows={3} placeholder="Réponse visible dans l'espace client..." className="mb-3" />
